    ...
    >
      <template v-slot:activator="{ on }">
          <div v-on="on" style="cursor: pointer;" class="text-center py-5 ma-1 fill-height" :class="$vuetify.theme.dark == true?'darkModeCard':'whiteTheme'" >
               <v-avatar size="100">
                ...
                <p class="mt-3 mb-0 roboto-font mb-0" style="font-size:120%">{{data.name | summery(20)}}</p>
                <p class="mt-1 mb-0 roboto-font mt-0" style="font-size:80%">{{data.designation | summery(20)}}</p>
      ...
      <v-card :class="theme.isDark?'grey darken-3':'white'">
        ...
        <v-card-text class="pa-5">
            <p class="roboto-font">{{data.designation}}</p>
